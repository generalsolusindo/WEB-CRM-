...
export default function Show({ salesOrder, totals, approvalDocs = [], canManageDocs = false, orderTypeLabel, paymentRuleLabel, requiredSettlementPhase, canCloseAsWon }) {
    ...
    function closeAsWon() {
        setClosing(true);
        feedback.expect({ success: { title: 'Selamat, deal dimenangkan!', style: 'popup' } });
    ...
    return <AppLayout><Head title={number} /><div className="mx-auto max-w-6xl space-y-5">
        ...
        <section className="card overflow-hidden p-0"><div className="border-b border-border px-5 py-4"><h2 className="font-semibold text-text">Item Sales Order</h2></div><TableScroll><table className="w-full text-left text-sm"><thead><tr className="border-b border-border bg-surface-2 text-[11px] font-bold uppercase tracking-wider text-text-faint"><th className="sticky left-0 z-[1] bg-surface-2 px-4 py-3">Item</th><th className="px-4 py-3">Qty</th><th className="px-4 py-3 text-right">Harga Jual</th><th className="px-4 py-3 text-right">Diskon</th><th className="px-4 py-3">Pajak</th><th className="px-4 py-3 text-right">DPP</th></tr></thead><tbody className="divide-y divide-border">{salesOrder.lines.map((line) => <tr key={line.id}><td className="sticky left-0 z-[1] bg-surface min-w-40 px-4 py-3"><div className="font-medium text-text">{line.item_name}</div><div className="whitespace-pre-line text-xs text-text-muted">{line.description || '—'}<CategoryBadge category={line.category} /></div>{line.sourcing_note && <div className="mt-0.5 text-[11px] italic text-text-muted">Opsi: {line.sourcing_note}</div>}</td><td className="px-4 py-3 text-text-muted">{line.qty} {line.unit}</td><td className="px-4 py-3 text-right">{money(line.selling_price)}</td><td className="px-4 py-3 text-right text-text-muted">{Number(line.discount_amount) > 0 ? `${money(line.discount_amount)} (${line.discount_percent ?? 0}%)` : '—'}</td><td className="px-4 py-3 text-text-muted">{line.tax ? line.tax.name : (Number(line.tax_rate) > 0 ? `${line.tax_rate}%` : '—')}</td><td className="px-4 py-3 text-right font-medium">{money(line.subtotal)}</td></tr>)}</tbody></table></TableScroll><TotalsSummary totals={totals} /></section>
