    ...
    return (
        <AppLayout>
            ...
            <div className="mx-auto max-w-4xl space-y-5">
                <div>
                    <h1 className="text-2xl font-bold text-text">Scope of Work</h1>
                    <p className="text-sm text-text-muted">SOW untuk project yang dikerjakan tim Anda — tanda tangani sebagai PIC Vendor.</p>
                </div>
                        ...
                        <tbody className="divide-y divide-border">
                            {sows.data.map((s) => (
                                <tr key={s.id} className="hover:bg-bg/70">
                                    ...
                                    <td className="px-4 py-3 text-right"><Link href={`/vendor/sows/${s.id}`} className="font-medium text-info hover:underline">Lihat</Link></td>
                                </tr>
                            ))}
                            {sows.data.length === 0 && <tr><td colSpan={4} className="px-4 py-12 text-center text-text-muted">Belum ada SOW.</td></tr>}
                        </tbody>
                    </table>
