...
/**
 * Regresi untuk bug: badge menu "SOW Menunggu TTD" milik Management menghitung SEMUA SOW
 * berstatus pending_director_signature, padahal daftar (Management\SowController::index)
 * hanya menampilkan yang project-nya belum didelegasikan ke Project Manager mana pun.
 * Akibatnya Management melihat badge "1" tapi daftarnya kosong — dilaporkan lewat screenshot.
 */
class SowMenuBadgeTest extends TestCase
{
    use RefreshDatabase;
    ...
    private function pendingDirectorSignatureSow(?User $pm): Sow
    {
        Storage::fake('local');
    ...
    public function test_badge_matches_the_list_when_project_is_delegated_to_a_pm(): void
    {
        ...
        $response = $this->actingAs($management)->get('/management/sows')->assertOk();

        // Daftar kosong karena sudah didelegasikan ke PM — badge harus ikut 0, bukan 1.
        $response->assertInertia(fn ($page) => $page->has('sows.data', 0));
        $this->assertSame(0, $response->viewData('page')['props']['auth']['menuBadges']['/management/sows']);
    }

    public function test_badge_matches_the_list_when_project_is_not_delegated(): void
    {
        ...
        $response->assertInertia(fn ($page) => $page->has('sows.data', 1));
        $this->assertSame(1, $response->viewData('page')['props']['auth']['menuBadges']['/management/sows']);
    }
}
